...
## Native Notes and Directory Overrides

Native Chain Notes uses `<os.config_dir()>/webview-app/notes.json`, with schema
version 1, validation, mutex protection and temporary-file replacement. Both
Notes and Quiz honor `WEBVIEW_APP_DATA_DIR`: setting it to `/tmp/my-profile`
places their files under `/tmp/my-profile/webview-app/`. The default location
is unchanged. Browser records are not automatically imported into native stores.

Native updates are debounced and serialized; selecting another note or leaving
the workspace flushes pending edits. Abrupt process termination and failed saves
after unmount still require draft recovery and application-wide close handling.
...
| --- | --- | --- |
| Chain Notes (browser mode) | `webview-app.chain-notes` | Note title, tag, question, answer, and update marker |
...
and the native Notes/Quiz files are separate stores.
...
Inside the native shell, Download PDF sends generated bytes through `save_pdf`
and saves under the user’s `Documents` directory, with numbered filenames on
collision and a 16 MiB decoded limit. Notes uses jsPDF text output; Academic
Paper uses html2pdf output. A native destination picker is not implemented.
